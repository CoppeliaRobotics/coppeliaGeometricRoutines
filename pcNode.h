...
public:
    CPcNode();
    CPcNode(CPcStruct* pc,double boxS,const C3Vector& boxCenter,double cellS,int cellPts,const std::vector<double>& points,std::vector<size_t>& ptsOriginalIndices,std::vector<bool>& ptsInvalidityIndicators,const std::vector<unsigned char>& rgbaData,bool rgbForEachPt);
    virtual ~CPcNode();
    ...
    void deserialize_ver2(CPcStruct* pc,const unsigned char* data,int& pos);
    void deserialize_float(CPcStruct* pc,const unsigned char* data,int& pos);

    size_t countCellsWithContent() const;
    void resetAllIds(CPcStruct* pc);
    void getDisplayPointsColorsAndIds(CPcStruct* pc,double boxS,const C3Vector& boxCenter,std::vector<float>& thePts,std::vector<unsigned char>& theRgbas,std::vector<unsigned int>& theIds) const;
    void getPointsPosAndRgba_all(double boxS,const C3Vector& boxCenter,std::vector<double>& data) const;
    void getPointsPosAndRgba_subset(double boxS,const C3Vector& boxCenter,double prop,std::vector<double>& data) const;
    ...
    void add_pts(CPcStruct* pc,double boxS,const C3Vector& boxCenter,double cellS,int cellPts,const std::vector<double>& points,std::vector<size_t>& ptsOriginalIndices,std::vector<bool>& ptsInvalidityIndicators,const std::vector<unsigned char>& rgbaData,bool rgbForEachPt);
